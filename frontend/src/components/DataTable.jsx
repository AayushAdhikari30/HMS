    ...
    <table className="w-full border-collapse">
      <thead>
        <tr>
          ...
          {onRowAction && (
            <th className="text-left text-[11px] font-bold uppercase tracking-widest text-[#666] px-5 py-3.5 bg-white/2 border-b border-[#1a1a1a]">
              Action
            </th>
          )}
        </tr>
      ...
      <tbody>
        {rows.map((row, idx) => (
          <tr key={row.id ?? idx} className="border-b border-[#1a1a1a] last:border-none hover:bg-white/2 transition-colors duration-100">
