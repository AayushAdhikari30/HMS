...
// --- Inline "write prescription" form ---
...
const AppointmentRow = ({ appt, onUpdateStatus, onStartComplete, onStartPrescribe, busyId, prescribed }) => {
    ...
    <tr className="border-b border-[#1a1a1a] last:border-none hover:bg-white/[0.02] transition-colors duration-100">
      ...
      <td className="px-5 py-3.5 align-middle">
        <div className="flex gap-2 flex-wrap">
          ...
          {appt.status === "confirmed" && (
            <>
              ...
              {prescribed ? (
                ...
                <button
                  onClick={() => onStartPrescribe(appt.id)}
                  disabled={busy}
          ...
          {appt.status === "cancelled" && <span className="text-xs text-[#444]">—</span>}
        </div>
      </td>
    </tr>
  );
...
const DoctorAppointments = () => {
  ...
  const submitPrescription = async (appt, { diagnosis, medications, notes }) => {
    setBusyId(appt.id);
    try {
      ...
      setPrescribed((prev) => ({ ...prev, [appt.id]: true }));
      setPrescribingId(null);
    } catch (err) {
      console.error("[DoctorAppointments] prescription failed:", err.message);
    } finally {
      setBusyId(null);
    }
  };
              ...
              filtered.map((appt) => (
                <Fragment key={appt.id}>
                  <AppointmentRow
                    appt={appt}
                    busyId={busyId}
                    prescribed={!!prescribed[appt.id]}
                    onUpdateStatus={(id, status) => updateStatus(id, status)}
                    onStartComplete={(id) => setCompletingId(id)}
                    onStartPrescribe={(id) => setPrescribingId(id)}
                  />
                  ...
                  {prescribingId === appt.id && (
                    <PrescribeEditor
                      saving={busyId === appt.id}
                      onDismiss={() => setPrescribingId(null)}
                      onSave={(payload) => submitPrescription(appt, payload)}
                    />
                  )}
                </Fragment>
              ))}
